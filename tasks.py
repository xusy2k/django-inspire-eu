...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-inspire-eu runtests.py django-example")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-inspire-eu.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ inspire_eu")
...
def lint(c):
    ...
    """
    c.run("flake8 django-inspire-eu django-example")
...
def release(c, bumpsize=""):
    ...
    c.run(
        'git tag -a {version} -m "New version: {version}"'.format(
            version=django_inspire_eu.__version__,
        ),
    )
    c.run("git push --tags")
